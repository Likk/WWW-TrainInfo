...
use Test::More;
use Test::Exception;
use WWW::TrainInfo::Plugin::TokyoMetro;
use Time::Piece 1.20 ();
...
sub prepare {
  ...
  {
    no strict 'refs';
    *{"WWW::TrainInfo::Plugin::TokyoMetro::mech"} = $code;
  }
...
subtest 'basic test' => sub {
  ...
  my $w = WWW::TrainInfo::Plugin::TokyoMetro->new( notify_no_delay => 1 );

  isa_ok $w,                'WWW::TrainInfo::Plugin::TokyoMetro', 'isa test';
  ...
  if(scalar(@$records) >= 1){
    for my $l (@$records){
      ...
      if($l->is_delay){
        like $l->description, qr/(?:【(.*)?遅延】|遅れが出ています。)/,      'delay ok';
      }
      elsif($l->is_stop){
        like $l->description, qr/運転を見合わせています/,  'stop ok';
      }
      elsif($l->is_cancel){
        like $l->description, qr/運休しています/,  'cancel ok';
      }
      elsif($l->is_normal){
        like $l->description, qr/現在、平常どおり運転しています。/, 'normal ok';
      }
      else{
        TODO: {
